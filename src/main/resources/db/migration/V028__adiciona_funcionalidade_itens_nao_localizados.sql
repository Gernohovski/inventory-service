INSERT INTO funcionalidade (funcionalidade, endpoint, http_method) VALUES
    ('BUSCAR_NAO_LOCALIZADOS', '/audit-service/v1/auditoria/ativa/itens-nao-localizados', 'GET');
...
SELECT
    ...
    (SELECT id FROM funcionalidade WHERE funcionalidade = 'BUSCAR_NAO_LOCALIZADOS') as funcionalidade_id
WHERE NOT EXISTS (
    SELECT 1 FROM funcao_funcionalidade
    WHERE funcao_id = (SELECT id FROM funcao WHERE nome = 'OPERADOR')
    AND funcionalidade_id = (SELECT id FROM funcionalidade WHERE funcionalidade = 'BUSCAR_NAO_LOCALIZADOS')
);