...
public record ItemController(ItemService itemService) {
	...
	public ResponseEntity<ItemResponseDTO> buscarItemPeloCodigo(@PathVariable("codigoItem") String codigoItem,
			@RequestHeader("X-ACCESS-TOKEN") String accessToken) {
	...
	@GetMapping("/{id}")
	public ResponseEntity<ItemDetalhadoResponseDTO> buscarItem(@PathVariable("id") Long itemId,
			@RequestHeader("X-ACCESS-TOKEN") String accessToken) {
